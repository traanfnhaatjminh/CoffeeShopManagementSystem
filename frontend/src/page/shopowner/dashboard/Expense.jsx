...
import { getListIngredient } from '../../../store/revenue-slice/revenueSlice';
...
const Expense = () => {
  ...
  const handleGetDataTotalRevenue = async () => {
    ...
    try {
      ...
      dispatch(getListIngredient(params)).then((ingredientOrder) => {
        setListIngredient(ingredientOrder.payload.data);
      });
    } catch (error) {
      console.log(error);
    } finally {
      ...
      <div className="container mx-auto p-6 bg-white shadow-lg rounded-xl font-mono">
        <div className="flex justify-between px-5 mb-4">
          ...
          <h1 className="text-3xl font-semibold text-gray-800">
            Chi phí đầu tư {filterMode === 'latest' ? `- Tháng ${month}` : ''}
          </h1>
          <select
            value={filterMode}
        ...
        <Table title="Danh sách nguyên liệu đã nhập" data={listIngredient} type="listIngredient" sum="expense" />
      </div>
    </>
  );
};
